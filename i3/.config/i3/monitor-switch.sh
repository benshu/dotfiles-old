...
then
    ...
    /usr/bin/notify-send --urgency=low -t 5000 "Graphics Update" "Auto screen layout"
...
then
    source /home/hagay/.screenlayout/external-monitors.sh
    /usr/bin/notify-send --urgency=low -t 5000 "Graphics Update" "Docked!"
    $HOME/.config/polybar/launch.sh
elif [ "$DIALOG_RESULT" = "Dock+Laptop" ];
then
    source $HOME/.screenlayout/docked-full-setup.sh
    /usr/bin/notify-send --urgency=low -t 5000 "Graphics Update" "Docked with Internal Screen!"
    $HOME/.config/polybar/launch.sh
